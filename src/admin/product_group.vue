    ...
    <el-main>
      ...
      <el-dialog
        ...
        <div style="position:relative;">
          群組名稱:
          <el-input v-model="product_dialog.caption" style="width:150px"placeholder="請輸入群組名稱"></el-input>
        </div>
        <div style="position:relative;">
          群組分類:
          <el-select class="none_delete_tag" ref="dialog_pro_tag" v-model="product_dialog.tag" 
          filterable remote multiple style="width:300px;" @focus="make_blur('0')" placeholder="請選擇分類"></el-select>
        </div>
        <div style="position:relative;">
          產品清單:
          <el-select class="none_delete_tag" ref="dialog_pro_group" v-model="product_dialog.product_list" 
          filterable remote multiple style="width:300px;" @focus="make_blur('1')" placeholder="請勾選產品"></el-select>
        </div>
        <div style="position:relative;">
          描述:
          <el-input v-model="product_dialog.description" style="width:200px" placeholder="請輸入描述"></el-input>
        </div>
        <div style="position:relative;">
          備註:
          <el-input v-model="product_dialog.remarks" style="width:200px" placeholder="請輸入備註"></el-input>
        </div>
        <span>
      ...
      <el-table :data="tableData">
        ...
        <el-table-column align="center" prop="operator" label="操作" width="150">
          ...
          <el-dialog
                ...
                <div style="position:relative;">群組分類:<el-select class="none_delete_tag" ref="dialog_pro_tag" v-model="product_dialog.tag" filterable remote multiple style="width:300px;" @focus="make_blur('0')" placeholder="請選擇分類"></el-select></div>
                <div style="position:relative;">產品清單:<el-select class="none_delete_tag" ref="dialog_pro_group" v-model="product_dialog.product_list" filterable remote multiple style="width:300px;" @focus="make_blur('1')" placeholder="請勾選產品"></el-select></div>
  ...
  export default {
    ...
    created(){
      this.axios.get("/admin_product_group",{
          ...
          this.module_data = JSON.parse(JSON.stringify(this.all_Data['module_data']))
          //------------------------將產品換成caption----------------------------//
          ...
          for(let mod of this.module_data)
            this.module_map.set(mod.mod_uid,{'mod_uid':mod.mod_uid, 'caption':mod.caption})
          var temp_pro_list = []
          for(let i = 0;i < this.product_group_data.length; i++){
            temp_pro_list = []
            for(let pro of JSON.parse(this.product_group_data[i]['product_list'])){
              if(this.caption_map.get(pro)){
              ...
              else if(this.module_map.get(pro)){
                temp_pro_list.push(this.module_map.get(pro)['caption'])
              }
            }
